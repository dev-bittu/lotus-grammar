...
In **mylang**, loops are defined using curly braces `{}` to enclose the block of code that should be repeated.
...
The `for` loop in **mylang** is used to iterate over a sequence (like a list, tuple, string, or dictionary) and execute a block of code for each item in the sequence. The loop automatically iterates through each element in the sequence, making it ideal for looping over collections.
...
In **mylang**, you can use a `range` to create a sequence of numbers, which is useful for looping a specific number of times.
